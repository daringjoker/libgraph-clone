...
void _boundaryfill(int x, int y, int color)
{
  ...
  if (x > screen->w || x < 0 || y > screen->h || y < 0)
    return;
  curr = getpixel(x, y);
  if ((curr != getfillcolor()) && (curr != color))
  {
    mapfillpixel(x, y);
...
void setlinestyle(int linestyle, int upattern, int thickness)
{
  ...
  case SOLID_LINE:
    ...
    break;
  case DOTTED_LINE:
    _internal_linestyle.upattern = 0x3333;
    break;
  case CENTER_LINE:
    _internal_linestyle.upattern = 0x0c3f;
    break;
  case DASHED_LINE:
    _internal_linestyle.upattern = 0x1f1f;
    break;
  case USERBIT_LINE:
    _internal_linestyle.upattern = (uint16_t)upattern;
    break;
  default:
    _internal_linestyle.upattern = 0xffff;
    break;
