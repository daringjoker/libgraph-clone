...
void initgraph(int *graphdriver, int *graphmode, char *pathtodriver)
{
  ...
  SDL_WM_SetCaption("libgraph modded by Pukar Giri", 0);
...
char *getmodename(int mode_number)
{
  ...
  {
  case VGALO:
    modename = strdup("640 x 200 VGA");
    break;
  case VGAMED:
    modename = strdup("640 x 350 VGA");
    break;
  case VGAHI:
    modename = strdup("640 x 480 VGA");
    break;
  case VGAMAX:
    modename = strdup("800 x 600 VGA");
    break;
  case VGA640:
    modename = strdup("640 x 480 VGA - fullscreen");
    break;
  case VGA800:
    modename = strdup("800 x 600 VGA - fullscreen");
    break;
  case VGA1024:
    modename = strdup("1024 x 800 VGA - fullscreeen");
    break;
  default:
    modename = strdup("User specified");
  }
  return modename;
}
...
{
  _fillcolor = colortrans(color);
  _curfillstyle = style;
}
int getfillcolor()
{
  return colorrev(_fillcolor);
}